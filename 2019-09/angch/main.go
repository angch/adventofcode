...
func main() {
	if false {
		// Part 1
		program := ParseIntCode("109,1,204,-1,1001,100,1,100,1008,100,16,101,1006,101,0,99")
		output := make(chan int)
		go IntCodeVM(program, output, output)
		for k := range output {
			fmt.Print(k, " ")
		}
		fmt.Println()

		program = ParseIntCode("1102,34915192,34915192,7,4,7,99,0")
		output = make(chan int)
		go IntCodeVM(program, output, output)
		for k := range output {
			fmt.Print(k, " ")
		...
		program = ParseIntCode("104,1125899906842624,99")
		output = make(chan int)
		go IntCodeVM(program, output, output)
		for k := range output {
			fmt.Print(k, " ")
